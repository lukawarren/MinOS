    ...
    m_Width(width), m_Height(height), m_X(x), m_Y(y), m_sTitle(title)
{
    ...
    m_pWidgets[4] = new Text("X");

    // Dummy terminal
    m_pWidgets[5] = new Panel(0);
    m_pWidgets[6] = new Text("colonel@minos /home $");
...
{
    // Bar
    m_pWidgets[0]->Redraw(m_Width, nBarHeight, m_X, m_Y);

    // Background panel
    m_pWidgets[1]->Redraw(m_Width, m_Height - nBarHeight, m_X, m_Y + nBarHeight);

    // Bar title
    m_pWidgets[2]->Redraw(m_X+nPadding, m_Y + nBarHeight/2 - CHAR_HEIGHT/2);
    ...
    m_pWidgets[5]->Redraw(terminalWidth, terminalHeight, terminalX, terminalY);
    m_pWidgets[6]->Redraw(terminalX + nPadding, terminalY + CHAR_HEIGHT/2);
}
